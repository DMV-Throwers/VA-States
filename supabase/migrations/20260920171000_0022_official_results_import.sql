-- ─────────────────────────────────────────────────────────────────────────────
-- Adds an "official import" path for reconciled/final results entered directly
-- (e.g. transcribed from an external NYYL scoring sheet after the event)
-- rather than submitted live by an authenticated judge through the clicker UI.
--
-- The live judging formula always normalizes a judge's own top raw Tech
-- Execution score to the division cap, which cannot reproduce a genuine
-- final result whose top scorer landed below the cap (e.g. multiple judges
-- averaged, or the source sheet just reports final numbers directly). When
-- is_official_import is true, vsyc_results uses tech_execution_override /
-- final_score_override verbatim instead of recomputing via per-judge
-- raw/max normalization -- this also allows the negative final scores the
-- NYYL rubric permits (unlike the live formula's GREATEST(0, ...) floor).
-- ─────────────────────────────────────────────────────────────────────────────
