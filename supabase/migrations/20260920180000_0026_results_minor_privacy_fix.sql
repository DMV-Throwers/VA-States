...
-- name is.
--
-- NOTE: this view is queried directly by both the public /results page and
-- the staff-only /admin/results page (createAdminClient(), no per-request
-- viewer awareness) -- unlike run-order, which distinguishes staff vs.
-- public callers and shows staff full legal names. This fix necessarily
-- masks for both audiences since the view itself can't know who's asking.
-- If staff need full legal names in admin/results for future live events,
-- that page should be changed to query vsyc_scores directly (joined to
-- vsyc_registrations without this masking) rather than vsyc_results, or a
-- second admin-only view/RPC should be added.
-- ─────────────────────────────────────────────────────────────────────────────
